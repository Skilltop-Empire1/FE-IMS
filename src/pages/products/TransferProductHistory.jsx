...
import { useGetTransferLogProductsQuery } from '../../redux/APIs/productApi'

const TransferProductHistory = () => {
  const {
    data: transferLogsData,
    error: transferLogsError,
    isLoading: isLoadingTransferLogs,
  } = useGetTransferLogProductsQuery()

  console.log({ transferLogsData })
  ...
  // Ensure transfers is an array or fallback to empty array
  const transfers = Array.isArray(transferLogsData?.transfers)
    ? transferLogsData.transfers
    : []
  ...
  if (isLoadingTransferLogs) return <p>Loading...</p>
  if (transferLogsError) return <p>Error loading transfer logs</p>
    ...
    <div className="container mx-auto px-4">
      <div className="flex justify-between items-center py-4">
        ...
        <button
          ...
          className="bg-imsPurple text-white px-4 py-2 rounded-md"
        >
          Back to Products
        </button>
      </div>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
            <button
              ...
              className={`px-3 py-1 border rounded-md ${
                page === currentPage ? 'bg-imsPurple text-white' : ''
              }`}
